...
import com.github.weixin.demo.domain.WebSocketBean;
...
//        JSONObject json=JSONObject.fromObject(message);
        WebSocketBean webSocketBean = new Gson().fromJson(message, WebSocketBean.class);

//        JSONObject json = (JSONObject) JSONObject.stringToValue(message);
        String string = null;  //需要发送的信息
        String to = null;      //发送对象的用户标识

        send(webSocketBean.getMessage(),
           role,
            webSocketBean.getRole(),
            webSocketBean.getSocketId());
    }
